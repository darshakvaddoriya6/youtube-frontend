...
const SubscriptionCard = ({ 
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 hover:shadow-md transition-shadow">
      {/* Mobile Layout */}
      <div className="flex flex-col lg:hidden p-4 space-y-3">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1 min-w-0">
            <h3
              className="font-semibold text-base text-gray-900 truncate cursor-pointer hover:text-red-600 transition-colors"
              onClick={() => onChannelClick(channel.username)}
            >
              {channel.fullName || channel.username}
            </h3>
            <p
              className="text-gray-600 text-sm cursor-pointer hover:text-red-600 transition-colors"
              onClick={() => onChannelClick(channel.username)}
            >
              @{channel.username}
            </p>
        ...
        <button
          ...
          className="w-full px-4 py-2.5 bg-red-600 text-white text-sm font-medium rounded-full hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
      ...
      <div className="hidden lg:flex items-center space-x-4 p-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between">
            <div className="flex-1 min-w-0">
              <h3
                className="font-semibold text-lg text-gray-900 truncate cursor-pointer hover:text-red-600 transition-colors"
                onClick={() => onChannelClick(channel.username)}
              >
                {channel.fullName || channel.username}
              </h3>
